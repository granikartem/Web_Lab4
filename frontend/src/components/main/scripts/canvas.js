...
function clicked(e, r, setChecks) {
    ...
    let width = cnvs.getAttribute("width");
    let height = cnvs.getAttribute("height");
    console.log(width);
    console.log(height);
    let event_x = e.pageX - cnvs.offsetLeft;
    let event_y = e.pageY - cnvs.offsetTop;
...
function drawCanvas(context, r, checks) {
    ...
    context.fill();
    context.moveTo(width /2, height / 2);
    context.arc(width / 2, width / 2, width / 5, -Math.PI/2, Math.PI /2);
    context.fill();
    context.beginPath();
    ...
    context.strokeText(rhalf_minus_text, width / 2, height / 5 * 3.5);
    context.strokeText(r_minus_text, width / 2, height / 5 * 4.5);
    context.strokeText(rhalf_minus_text, width / 5 * 1.5, height / 2);
    context.strokeText(rhalf_text, width / 2, height / 5 * 1.5);
    context.strokeText(r_text, width / 2, height / 10);
    context.strokeText(r_minus_text, width / 10, height / 2);
    context.strokeText("Y", width / 1.97, height / 30);
...
function drawPoint(x, y, r, result, rval, context) {
    ...
    let width = context.canvas.width;
    let height = context.canvas.height;
    let canvasX = width/2 + x_Value * (width / 5 * 2) / r_valValue;
    let canvasY = height/2 - y_Value * (height / 5 * 2) / r_valValue;
    if (result === "false") {
